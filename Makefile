...
.PHONY: build build-backend build-datanode-ui \
	run run-backend run-datanode-ui run-docker \
...
help:
	...
	@echo "  make run            Run backend (Spring Boot) – serves built datanode-ui; use run-datanode-ui for dev server"
	...
	@echo ""
	@echo "  make run-backend      Run datanode Spring Boot app (requires DB; use run-docker for full stack)"
	@echo "  make run-datanode-ui  Run datanode-ui dev server (npm start)"
...
run-backend: build-backend
	mvn -q spring-boot:run -pl datanode -am

run-datanode-ui:
	-lsof -ti:3000 | xargs kill -9 2>/dev/null || true
	cd datanode-ui && npm run start
