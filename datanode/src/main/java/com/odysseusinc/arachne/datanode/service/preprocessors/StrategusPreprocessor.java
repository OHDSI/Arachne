...
public class StrategusPreprocessor implements Preprocessor<Analysis> {
    ...
    public void preprocess(Analysis analysis, File file) {
        FileSystemResource iss = new FileSystemResource(file.getAbsolutePath());
        Optional.ofNullable(StrategusMatcher.INSTANCE.getContentType(file.getName(), iss)).ifPresent(__ ->
                preprocess0(analysis, file)
        );
    }
    ...
    private void preprocess0(Analysis analysis, File file) {
        try {
            ...
            log.error("Failed to preprocess Strategus JSON [{}]:", file.getName(), e);
