...
import { performRegister } from "../../store/modules";
...
export const RegisterPage: React.FC<RegisterPageProps> = ({ onBackToLogin }) => {
  ...
  const registerError = useSelector<any, string>((state: any) => state.authMode.registerError);

  useEffect(() => {
    document.title = "Arachne Data Node - Register";
    getPasswordPolicy()
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    dispatch(performRegister(username, password));
  };
                ...
                <Grid item xs={12} spacing={2} container>
                  ...
                  <Grid item xs={12} textAlign="right">
                    <Button
                      ...
                      type="submit"
                      fullWidth
                      disabled={!username || !password || !confirmPassword || !passwordPolicy || registerStatus === Status.IN_PROGRESS}
                    >
                      {registerStatus === Status.IN_PROGRESS ? (
                        <>
                          <LogInText>Registering...</LogInText><Spinner size={18} />
                        </>
                      ) : (
                        <>Create Account</>
                      )}
